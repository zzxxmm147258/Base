package com.hibo.bas.http.httpClient;
...
public class HttpProtocolHandler {
    ...
    /** 闲置连接超时时间, 由bean factory设置，缺省为0秒钟 */
//    private int                        defaultIdleConnTimeout              = 0*1000;
//    
//    private int                        timeoutInterval              	   = 1*1000;
...
//        ict.setTimeoutInterval(timeoutInterval);
//        ict.start();
    }
	...
	public HttpResponse execute(HttpRequest request, String strParaFileName, String strFilePath) throws HttpException, IOException {
        ...
        if (request.getMethod().equals(HttpRequest.METHOD_GET)) {
        ...
        } else if(StrUtil.isnull(strParaFileName)&& StrUtil.isnull(strFilePath)) {
        	//post模式且不带上传文件
            ...
            if(null!=request.getRequestBody()){
            	Object p = request.getRequestBody();
            	if(p instanceof String){
            		((PostMethod) method).setRequestBody((String)p);
            	}else if(p instanceof NameValuePair[]){
            		((PostMethod) method).setRequestBody((NameValuePair[])p);
            	}else if(p instanceof InputStream){
            		((PostMethod) method).setRequestBody((InputStream)p);
            	}
            }
