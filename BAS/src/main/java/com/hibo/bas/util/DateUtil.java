 ...
 * <p>公司: 北京瀚铂科技有限公司</p>
 * <p>创建日期：2015年7月4日 下午7:05:15</p>
 * <p>类全名：com.hibo.bas.util.DateUtil</p>
 * 作者：陈国红
 * 初审：
 * 复审：
 * @version 1.0
...
public class DateUtil {
	...
	private static Calendar newGregorianCalendar() {
		/*
		 * TimeZone.getTimeZone("GMT+8:00")
		 */
		final int hOffset = TimeZone.getDefault().getRawOffset()
				/ (60 * 60 * 1000);
		/*
		 * 对于北京 时区， hOffset = +8 Calendar.getInstance() 改为
		 * Calendar.getInstance(TimeZone
		 * .getTimeZone("GMT"+(hOffset>0?"+"+hOffset:""+hOffset)+":00")); 为了避免
		 * 1988-1991 使用夏令时的问题 Daylight Saving Time：DST 因为
		 * toLongTime(year,month,day,....)
		 */
		return Calendar.getInstance(TimeZone.getTimeZone("GMT"
				+ (hOffset > 0 ? "+" + hOffset : "" + hOffset) + ":00"));
	}

	final private static Calendar gmt0Cal = Calendar
			.getInstance(java.util.TimeZone.getTimeZone("GMT+0"));
	 ...
	 */
	private static Calendar getStaticCalendars(java.util.Date date) {
		if (date != null)
	...
	public static int getDateMonth(java.util.Date date) {
		return date == null ? 0 : date.getMonth() + 1;
	}
	...
	/**
	 * @return date.getDay() 周1 ： turn 1, 2 : return 0,
	 */
	...
	/*
	 * 从 fromDate 日开始， date 所在 周，（fromDate 当前算 第 0 周 ) 计算在当前月份中的周： y = 2015, m =
	 * 4, d = 7
	 */
	public static int indexOfWeek(java.util.Date date, java.util.Date fromDate) {
		...
		if (days >= 0) {
			if (w0 == 0)
				...
				return days < 7 - w0 ? 0 : 1 + (days - (7 - w0)) / 7; // days<7-w0
																		// ? 0 :
		} else {
			if (w0 == 0)
				return -1 - (-days - 1) / 7;
			else
				return -days <= w0 ? 0 : -1 - (-days - 1 - w0) / 7; // days<7-w0
																	// ? 0 :
		}
	}
	...
	// 1.1 only ,
	// 1.2 : (java.util.Date)(date.clone()) instead
	/**
	 * 从一个<code>java.util.Date</code>对象clone一个相同类型的对象. 目前只考虑 java.util.Date,
	 * java.sql.Date , java.sql.Timestamp 三中类型
	 * 
	...
	/**
	 * @param value
	 *            java.util.Date 或 String("2000-9-7", "2002-6-1 14:15" ,
	 *            "2002-6-1 14:15:30") 得到一个Date对象
	 */
	...
	/*
	 * public static java.util.Date toDateTime(String text) { if(text==null)
	 * return null; text = text.trim(); if( text.length()==0 ) return null; int
	 * r[] = new int[6]; if( !DateUtil.parseDateTime(text,r) ) return null;
	 * return DateUtil.toDate(r[0],r[1],r[2]); }
	 */
	// public static java.util.Date obj2date(Object o)
	// {
	//
	// }
	/**
	...
	public static long toLongTime(int year, int month, int day, int hour,
			int min, int sec) {
		// if(staticCal==null) staticCal = newGregorianCalendar();//new
		// GregorianCalendar();
		synchronized (staticCal) {
			staticCal.clear();
			staticCal.set(year, month - 1, day, hour, min, sec);
			// //Calendar.YEAR,
			// staticCal.set(Calendar.MONTH,month-1);
			// staticCal.set(Calendar.DAY_OF_MONTH,day); // day-1??
	...
	public static long time0ForToday(long today) {
		// java.util.Calendar date = newGregorianCalendar();//new
		// java.util.GregorianCalendar();
		synchronized (staticCal) {
			staticCal.setTime(new java.util.Date(today));
			staticCal.set(staticCal.get(Calendar.YEAR),
					staticCal.get(Calendar.MONTH),
					staticCal.get(Calendar.DAY_OF_MONTH), 0, 0, 0);
			staticCal.set(Calendar.MILLISECOND, 0);
			return staticCal.getTime().getTime();
		}
	}
	...
	public static long getTime0(java.util.Date date) {
		synchronized (staticCal) {
			java.util.Calendar cal = getStaticCalendars(date);// newGregorianCalendar();//new
																// java.util.GregorianCalendar();
			// cal.setTime(date);//new java.util.Date(today));
	...
	public static java.util.Date today0() {
		return toDate0(new java.util.Date());
	}

	public static java.util.Date toDate0(java.util.Date aDate) {
		if (aDate == null)
			return null;
		synchronized (staticCal) {
			java.util.Calendar date = getStaticCalendars(aDate);// new
	...
	public static long getDateTime(Object date) {
		return date instanceof java.util.Date ? ((java.util.Date) date)
				.getTime() : 0;
	}

	static public long deltaFromServer = Long.MIN_VALUE;
	static public long serverStartTime = Config.startTime;
	 ...
	 * @param month
	 *            月
	 * @return 给定年、月的最大天数(1月返回31,2月返回28或29,3月返回31,...,12月返回31) for m = 1 to 12 n
	 *         = DateUtil.getDaysOfMonth(2015,m) println( m+" : "+n ) end for
	 *         println (java.util.TimeZone.getTimeZone("GMT+0") )
	 */
	static public int getDaysOfMonth(int year, int month) {
		final long t1, t0;
		// 2015-04-08 : 由于夏令时的问题， 改为 使用 gmt0Cal
		synchronized (gmt0Cal) {
			gmt0Cal.clear();
			// staticCal.set(year,month-1,day,hour,min,sec);
			gmt0Cal.set(month == 12 ? (year + 1) : year, month == 12 ? 0
					: month, 1, 0, 0, 0);
			t1 = gmt0Cal.getTimeInMillis();
			gmt0Cal.clear();
			gmt0Cal.set(year, month - 1, 1, 0, 0, 0);
			t0 = gmt0Cal.getTimeInMillis();
		}
		return (int) ((t1 - t0) / (24 * 60 * 60 * 1000l));
	}
	...
	static public java.util.Date incMonth(java.util.Date date, int nMonth,
			int options) {
		...
		return new java.util.Date(toLongTime(e[0], e[1], e[2], e[3], e[4],
				e[5], e[6]));
	}

	static public java.util.Date incYear(java.util.Date date, int nYear,
			int options) {
		if (date == null || nYear == 0)
			return date;
		int e[] = DateUtil.getDateElements(date);
		e[0] += nYear;
		if ((options & 1) != 0) {
	 ...
	 * @return
	 */
	static public java.util.Date incSeconds(java.util.Date date, int seconds){
	...
	static public void incYearMonthDay(int ymd[], int idx0, int day) {
		java.util.Date date = incDate(
				toDate(ymd[idx0 + 0], ymd[idx0 + 1], ymd[idx0 + 2]), day);
	...
	static public int diffDate(Calendar date1, Calendar date2) {
		return (int) ((toLongTime(date1.get(Calendar.YEAR),
				date1.get(Calendar.MONTH) + 1, date1.get(Calendar.DATE)) - toLongTime(
				date2.get(Calendar.YEAR), date2.get(Calendar.MONTH) + 1,
				date2.get(Calendar.DATE))) / (24 * 60 * 60 * 1000));
		/*
		 * date1.set(Calendar.HOUR,0); date1.set(Calendar.MINUTE,0);
		 * date1.set(Calendar.SECOND,0); date1.set(Calendar.MILLISECOND,0);
		 * date2.set(Calendar.HOUR,0); date2.set(Calendar.MINUTE,0);
		 * date2.set(Calendar.SECOND,0); date2.set(Calendar.MILLISECOND,0);
		 * return
		 * (int)((date1.getTime().getTime()-date2.getTime().getTime())/(24
		 * *60*60*1000));
		 */
	}
	...
	static public final String calendarToString(final java.util.Calendar cal,
			int hmsFormat) {
		...
		if (hmsFormat == 5)
			return text + " " + toStr2(hour) + ":" + toStr2(min) + ":"
					+ toStr2(sec) + "." + cal.get(Calendar.MILLISECOND);
		if (hour == 0 && min == 0 && sec == 0)
			return text;
		return sec == 0 ? text + " " + toStr2(hour) + ":" + toStr2(min) : text
				+ " " + toStr2(hour) + ":" + toStr2(min) + ":" + toStr2(sec);
	}
	 ...
	 *            日期对象
	 * @param hmsFormat
	 *            0 : 1 ： 总是 yyyy-mm-dd hh:mm:ss 格式 2 : 只含年、月、日形式, 无论 4 :
	 *            yyyy-mm-dd hh:mm 5 ： 总是 yyyy-mm-dd hh:mm:ss.ms 格式(带毫秒)
	 * 
	 *            0x1X : yyyy 两位
	 * @return date的字符串表示
	 */
	static public final String dateToString(java.util.Date date, int hmsFormat) {
		...
		synchronized (staticCal) {
			if (hmsFormat == 0 && date instanceof java.sql.Date)
				hmsFormat = 2;
			return calendarToString(DateUtil.getStaticCalendars(date),
					hmsFormat);
		}
	}
	...
	static public String dateToEnglishString(java.util.Date date,
			boolean shortMonth, char deliMD, char deliDY) {
		if (date == null)
			return null;
		...
		String month;
		if (shortMonth)
			month = "JanFebMarAprMayJunJulAugSepOctNovDec".substring(m * 3,
					m * 3 + 3);
		else {
		...
		return month + deliMD + calendar.get(Calendar.DATE) + deliDY
	...
	/**
	 * 日期返回字符串 YYYYMMDD  或 YYMMDD
	 * @param date
	 * @param bYear
	 * @return
	 */
	public static String dateToString2(Date date,boolean bYear){
		Calendar cal = getStaticCalendars(date);
		int year = cal.get(Calendar.YEAR);
		int month = cal.get(Calendar.MONTH) + 1;
		int day = cal.get(Calendar.DAY_OF_MONTH);
		String yearText = Integer.toString(year);
		if (bYear && yearText.length() > 2)
			yearText = yearText.substring(yearText.length() - 2);
	 ...
	 * @return
	 */
	public static String dateToSeconds(Date date){
		Calendar cal = getStaticCalendars(date);
		int year = cal.get(Calendar.YEAR);
		year = year - 2000;
		int month = cal.get(Calendar.MONTH) + 1;
		...
		int dates = year*372 + month*31 + day;
		return Integer.toString(dates);
	}
	 ...
	 * @return
	 */
	public static String timeToSeconds(Date date){
		Calendar cal = getStaticCalendars(date);
		int hour = cal.get(Calendar.HOUR_OF_DAY);
		int min = cal.get(Calendar.MINUTE);
		int sec = cal.get(Calendar.SECOND);
		int dates = hour*3600 + min*60 + sec;
		return Integer.toString(dates);
	}
	...
	public final static boolean parseDate(String text, int ret[],
		...
		if (caseSize8 && text.length() == 8 && isDigites(text)) {
			final int y = tmp[0] = Integer.parseInt(text.substring(0, 4));
			final int m = tmp[1] = Integer.parseInt(text.substring(4, 6));
			final int d = tmp[2] = Integer.parseInt(text.substring(6, 8));
			if (m <= 0 || m > 12 || y < 1000 || d <= 0
					|| d > DateUtil.getDaysOfMonth(y, m))
				return false;
		} else {
	...
	public final static java.util.Date[] parseDateRange(String text,
			java.util.Date today, boolean caseTime) {
		...
		if (p >= 0) {
			...
			return date1 != null || date2 != null ? new java.util.Date[] {
					date1, caseTime ? setDayEndTime(date2) : date2 } : null;
		}
		if (text.charAt(0) >= 'a' && text.charAt(0) <= 'z') {
			...
			} else if ("semiyear".equals(text)) // 半年
			{
				...
				if ("day".equals(text)) {
					...
					fromDate = total ? (after ? DateUtil.incDate(today, 1)
							: today) : toDate;
				} else if ("month".equals(text)) {
					int toMonth = thisMonth + n;
					int fromMonth = total ? (after ? thisMonth + 1 : thisMonth)
							: toMonth;
					...
					for (; toMonth > 13; toMonth -= 12, toYear++)
						;
					toDate = DateUtil.toDate(toYear, toMonth,
							DateUtil.getDaysOfMonth(toYear, toMonth));
					fromDate = DateUtil.toDate(fromYear, fromMonth, 1);
				} else if ("year".equals(text)) {
					int toYear = thisYear + n;
					int fromYear = total ? (after ? thisYear + 1 : thisYear)
							: toYear;
					if (toYear < fromYear) {
						int tmp = fromYear;
						fromYear = toYear;
					...
					final Date week1Date = DateUtil.incDate(today, wd == 0 ? -6
							: 1 - wd);
					// week1Date : 本周的 周一日期：
					int toWeek = n;
			...
			if (fromDate != null && toDate != null) {
				return toDate.getTime() < fromDate.getTime() ? new java.util.Date[] {
						toDate, caseTime ? setDayEndTime(fromDate) : fromDate }
		...
		return date == null ? null : new java.util.Date[] { date,
				caseTime ? setDayEndTime(date) : date };
	}
	...
	public final static Date parseDate(String text, String format) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (j >= 0) {
				...
				int d = c - '0';
				if (d < 0 || d > 9)
					throw new java.lang.IllegalArgumentException("日期格式错:"
							+ text);
				ymd[j] = ymd[j] * 10 + d;
			} else if (c != fc)
				throw new java.lang.IllegalArgumentException("日期格式错:" + text);
	...
	public final static String dateToString(Date date, String format) {
		...
		synchronized (staticCal) {
			...
			int ymd[] = new int[6];
			ymd[0] = cal.get(Calendar.YEAR);// ((java.sql.Timestamp)value).getYear()
											// + 1900;
			ymd[1] = cal.get(Calendar.MONTH) + 1;// ((java.sql.Timestamp)value).getMonth()
													// + 1;
	...
	public final static boolean parseDateTime(String text, int ret[]) {
		...
		if ((l == 8 || l == 10 || l == 12 || l == 14) && isDigites(text)) {
			...
			if (a == null || a.length < 5 || a.length > 13
					|| (a.length & 1) == 0)
				return false;
