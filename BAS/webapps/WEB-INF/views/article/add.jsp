  ...
  <form id="inputForm" class="form-horizontal" action="update" enctype="multipart/form-data"
   ...
   <form id="inputForm" class="form-horizontal" action="save" enctype="multipart/form-data" method="post">
			...
			<div class="col-sm-4">
			    <select id="category"  name="category"  class="form-control">
               		 <c:forEach items="${categorys }" var="category">
               		    <c:choose>
						<c:when test="${(not empty article.category) && (article.category eq category.code)}">
			...
			<div class="col-sm-4">
			    <select id="articleCategory"  name="articleCategory"  class="form-control">
               		  <c:forEach items="${articleCategorys }" var="articleCategory">
               		    <c:choose>
						<c:when test="${(not empty article.articleCategory) && (article.articleCategory eq articleCategory.code)}">
    	...
    	<div class="form-group"> 
			...
			<div class="col-sm-3">
				<select class="form-control attr4" name="attr4"  maxlength="200">
					<c:forEach items="${dictinfo}" var="dicfo">
				 		<c:choose>
				  			<c:when test="${(not empty article.attr4) && (article.attr4 eq dicfo.code)}">
    	...
    	<div class="form-group">
			...
			<div class="col-sm-10">
			<c:choose>
			  <c:when test="${empty article.seoDescription} ">
			    <input type="text" name="seoDescription"  maxlength="200"  style="height:80px" class="form-control" />
			  </c:when>
			  <c:otherwise>
			     <input type="text" name="seoDescription"  maxlength="200" style="height:80px" value="${article.seoDescription}"  class="form-control"/>
			  </c:otherwise>
			</c:choose>
			</div>
    	</div>
    	...
    	<div class="form-group" id="img_path">
		    ...
		    <div class="col-sm-10">
		    <c:choose>
			<c:when test="${empty article.contentUrl}">
		      <input type="text" name="contentUrl"  maxlength="200"   class="form-control" />
			</c:when>
			<c:otherwise>
			   <input type="text" name="contentUrl"  maxlength="200"  value="${article.contentUrl}"  class="form-control"/>
			</c:otherwise>
			</c:choose>
			</div>
		</div>
		...
		<table class="table">
			<tr>
				...
				<td><input type="submit" class="btn btn-default" value="确认" />
					...
					<c:if test="${not empty article.articleCategory}">
					<a  menuname="重置" href='<c:url value="/main/article/edit?id=${article.id}"/>'  class="btn btn-default" >重置</a>
					</c:if>
					</td>
			</tr>
		</table>
