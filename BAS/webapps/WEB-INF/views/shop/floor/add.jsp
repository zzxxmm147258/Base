...
<script type="text/javascript">
	$().ready(function (){
		...
		$('#buildingId').multiselect({
			enableFiltering:true,
			maxHeight: 290,
			buttonWidth: '100%',
		});
		
		var platformStr = "${bean.buildingId}";
		pf = new Array;
...
<div class="modal" id="myModal" role="dialog" aria-labelledby="myModalLabel">
	<form:form  id="inputForm" method="${method }" modelAttribute="bean" enctype="multipart/form-data">
        ...
        <div class="panel-body">
			<div class="row">
			   <div class="col-md-4">
    				<label>楼层号</label><form:errors path="floorNo" style="color:red;"/>
					 <form:input id="floorNo" path="floorNo" cssClass="form-control" /> 
    			</div>
    			
    			<div class="col-md-4">
    				<label>楼层名称</label><form:errors path="floorName" style="color:red;"/>
					 <form:input id="floorName" path="floorName" cssClass="form-control" /> 
    			</div>
    			
			    <div class="col-sm-4">
    			<label>街区</label><form:errors path="buildingId" style="color:red;"/>
    			<form:select id="buildingId" class="multiselect-search" path="buildingId">
    				 <c:forEach items="${buildingBas}" var="buildingBas">
						<form:option value="${buildingBas.buildingId}">${buildingBas.buildingName}</form:option>
					 </c:forEach>
				</form:select>
			    </div>
    		</div>
    		...
    		<div class="row">
				...
				<input type="submit" class="btn btn-primary"  onclick="return Info()" value="提交" /> <a
					href="<c:url value='/admin/floor/list'/>" class="btn btn-success">返回</a>
		</div>   
		</div>  
		</div>
		</div>   
